...
function gestionUsuario(tipo_gestion){
    var usuario = {
        ...
        password: document.getElementById("password-registro").value.trim()
    }

    if (usuario.nombre == "" || usuario.apellidos == "" || usuario.usuario == "" || usuario.password == "") {
...
function crearRecurso() {
    ...
    rest.post("/api/recursos", nuevoRecurso, function(estado, respuesta) {
        if (estado == 201) {
            alert(respuesta.mensaje);
            enviarAvisoRecurso(respuesta.id);
            actualizarTabla();
...
function enviarAvisoRecurso(idRecurso) {
