...
function guardarResenya() {
    ...
    crearResenya(id_recurso_resenya, id_sanitario, valoracion, texto, function(idResenya) {
        if (idResenya != null) {
            enviarAvisoResenya(id_recurso_resenya, valoracion);
...
function retirarRecurso(id, conReserva) { // se llama con el idReserva o idRecurso en funcion de si tiene o no reserva
    if (conReserva) { // se tiene reserva (aparece en el menu principal)
        ...
        iniciarReserva(idReserva, function (ok) {
            if (ok) {
                enviarAvisoReserva("inicio", idReserva);
                actualizarReservasPendientes();
                actualizarReservasRealizadas();
            }
            else {
        ...
        reservarRecurso(idRecurso, id_sanitario, Number(horas), function(respuesta) {
            if (respuesta.estado == "ok") {
                iniciarReserva(respuesta.idReserva, function(ok) {
                    if (ok) {
                        enviarAvisoReserva("inicio", respuesta.idReserva);
                        cambiarSeccion("menu-principal");
                    }
                    else {
...
function devolverRecurso(idReserva) {
    finalizarReserva(idReserva, function (ok) {
        if (ok) {
            enviarAvisoReserva("fin", idReserva);
            actualizarReservasPendientes();
            actualizarReservasRealizadas();
        }
        else {
...
function conectarWSSanitario() {
    if (ws != null) {
        ws.close();
    }

    ws = new WebSocket("ws://localhost:3502");

    ws.onopen = function() {
        var mensaje = {
            tipo: "login",
            rol: "sanitario",
            idUsuario: id_sanitario
        };
...
function actualizarTablaAvisos() {
    var tablaAvisos = document.getElementById("tabla-avisos");

    if (tablaAvisos == null) {
        return;
    }

    tablaAvisos.innerHTML = "";

    for (var i = 0; i < avisos.length; i++) {
        var clase = "aviso-" + avisos[i].color;

        tablaAvisos.innerHTML +=
            "<tr class='" + clase + "'>" +
                "<td>" + formatearFecha(avisos[i].fecha) + "</td>" +
                "<td>" + avisos[i].origen + "</td>" +
                "<td>" + avisos[i].texto + "</td>" +
            "</tr>";
    }
}

function enviarAvisoReserva(accion, idReserva) {
...
function enviarAvisoResenya(idRecurso, valoracion) {
    if (ws != null && ws.readyState == WebSocket.OPEN) {
        var mensaje = {
            tipo: "aviso_resenya",
            idRecurso: idRecurso,
